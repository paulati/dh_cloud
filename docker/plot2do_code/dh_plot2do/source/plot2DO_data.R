...
ConvertGRangesToDF <- function(data) {
  result <- data.frame(data, stringsAsFactors = FALSE)
  return(result)
}
  
ConvertDFToGranges <- function(data, genome, annotations) {
  # https://web.mit.edu/~r/current/arch/i386_linux26/lib/R/library/GenomicRanges/html/makeGRangesFromDataFrame.html
  # https://rdrr.io/bioc/GenomicRanges/man/makeGRangesFromDataFrame.html  
  # tengo muchos probemas si no respeto el orden original de seqlevels
  
  seqlevels_source <- levels(data$seqnames)
  reads <- makeGRangesFromDataFrame(data, seqinfo = seqlevels_source)
