...
CreateOptionsParser <- function(){
  ...
  options = list(
    ...
    make_option(c("--jobId"), type="character", default=NULL, 
                help="Identifies caller process")
  )
...
# sites <- params$referencePointsBed
# selectedReference <- params$reference
# siteLabel <- params$siteLabel
CreateOutputFolders <- function(jobId, plotType, sites, selectedReference, siteLabel) {
  # Create folder where figures are saved
  folderPath <- GetOutputBaseFolderPath(jobId, plotType, sites, selectedReference, siteLabel)
  dir.create(folderPath, showWarnings = FALSE, recursive = TRUE)
  return(folderPath)
}

# output helper functions:
GetOutputBaseFolderPath <- function(jobId, plotType, sites, selectedReference, siteLabel){
  ...
  folderPath <- file.path(outputBasePath, jobId, folderName)
  
  return(folderPath)
}

GetOutputPlotFilePath <- function(jobId, plotType, sites, selectedReference, siteLabel,
                                      lMin, lMax, sampleName)  {
  extension <- ".pdf"
  result <- GetOutputFilePathBase(jobId, plotType, sites, selectedReference, siteLabel, 
                                      lMin, lMax, sampleName, extension)
  return(result)
  
}

GetOutputMatrixFilePath <- function(jobId, plotType, sites, selectedReference, siteLabel,
                                        lMin, lMax, sampleName){
...
GetOutputFilePathBase <- function(jobId, plotType, sites, selectedReference, siteLabel, 
                                      lMin, lMax, sampleName, extension){
  
  folder_base_path <- GetOutputBaseFolderPath(jobId, plotType, sites, selectedReference, siteLabel)
