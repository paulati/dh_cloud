...
PlotAverageOccupancy <- function(occMatrix, beforeRef, afterRef, xTitle, yTitle, mainTitle, customTheme) {
  ...
  deltaYBreaks = max(0.1, round(1.05 * max(avgOcc)/10, digits = 1))
  yBreaks <- seq(0, 1.05 * max(avgOcc), deltaYBreaks)
  if (length(yBreaks) >= 8){
    yLabels <- FixTickLabels(as.character(yBreaks), 1, " ")  # yBreaks
  } else {
    yLabels <- FixTickLabels(as.character(yBreaks), 0, " ")  # yBreaks
  }
  yLimits <- c(0, 1.05 * max(avgOcc))
...
PlotFragmentLength <- function(lengthHist, lMin, lMax, xTitle, yTitle, customTheme) {
  ...
  yLabels <- yBreaks
  yLimits <- c(0, 1.05 * max(lengthHist))
...
PlotFigure <- function(params)  {
  ...
  dataFilePath <- GetOutputMatrixFilePath(params$jobId, params$plotType, params$referencePointsBed, 
  ...
  plotFilePath <- GetOutputPlotFilePath(params$jobId, params$plotType, params$referencePointsBed, 
